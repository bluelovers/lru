...
export type IEntry<KeyType, ValueType> =
	{
		key: KeyType,
		value: ValueType,
	}

export type IEntryEvent<KeyType, ValueType, EventType extends ILRUEvents> = IEntry<KeyType, ValueType> &
	{
		event: EventType,
	}

export const enum EnumLRUEvents
{
	evict = 'evict',
}

export type ILRUEvents = EnumLRUEvents | string | symbol;
...
export interface ILRU_Interface<KeyType, ValueType> extends EventEmitter, QuickLRU<KeyType, ValueType>
{
	...
	addListener(event: EnumLRUEvents.evict,
		listener: <T>(entry: IEntryEvent<KeyType, ValueType, EnumLRUEvents.evict>) => T,
	): this;

	on(event: EnumLRUEvents.evict, listener: <T>(entry: IEntryEvent<KeyType, ValueType, EnumLRUEvents.evict>) => T): this;

	once(event: EnumLRUEvents.evict,
		listener: <T>(entry: IEntryEvent<KeyType, ValueType, EnumLRUEvents.evict>) => T,
	): this;
